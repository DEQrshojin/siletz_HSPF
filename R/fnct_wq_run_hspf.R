...
read_wq_pars <- function(cf = NULL, filN = NULL, writeCsv = TRUE) {

  # Synopsis
  # Reads control file and populates control object passed to other functions
  # cf = control file, filN = file name
  ...
  v$emcFil <- paste0('C:/siletz_tmdl/03_models/01_hspf/emcdwc_', v$pars, '.csv')
  ...
  if (writeCsv) { # Write wq parameters & seasonality to .csv
    df <- write_wq_2_csv(pars = v$pars, tmat = v$tmat, SURO = v$SURO,
                         ...
                         AGW5 = v$AGW5, AGW6 = v$AGW6, filN = filN)
  }
...
write_wq_2_csv <- function(pars = NULL,  tmat = 1.000, SURO = 1.5, IFWC = 0,
                           ...
                           AGW4 = 0, AGW5 = 0, AGW6 = 0, filN = NULL) {
  ...
  write.csv(df, paste0('C:/siletz_tmdl/03_models/01_hspf/emcdwc_', pars, '.csv'),
            row.names = F, quote = F)
...
#_______________________________________________________________________________
run_wq <- function(v = NULL, hydros = NULL) {
  ...
  # If there are existing hydros (rDat in form of .RData read those)
  ...
  tTime <- 0; a <- as.numeric(Sys.time())

  # Load the model run number
  ctrF <- read_ctrF_H()

  #_____________________________________________________________________________
  # Load in the flow data - Read from RData file if provided
  qOut <- readRDS(hydros)
  ...
  cat(paste0('Time to load data: ', tTime, ' minutes\n'))
  ...
  if (length(yrs) == 1) {
    ...
    dts <- data.frame(str = as.POSIXct(c(v$strD, intDts), '%Y-%m-%d',
                                       tz = 'America/Los_Angeles'),
                      end = as.POSIXct(c(intDts, v$endD), '%Y-%m-%d',
                                       tz = 'America/Los_Angeles'))
  }

  # Delete the last row if the start and end dates are the same
...
proc_wq <- function(restart = NULL, strD = NULL, endD = NULL, v = NULL,
  ...
  # ****************************************************************************
  # Pre-process reach flows & loads (reduce to start/end dates)
  qRch <- reduce_qlc(strDte = strD, endDte = endD, df2Red = qOut[['qRch']])

  # Storage volume in the reach at the start of the timestep
  rchV <- qRch[, c(1, ((length(qRch) - 1) / 2 + 2) : length(qRch))] # Reach Vol (VOL)

  # Reach outflow (cubic meters per second)
  rchQ <- qRch[, 1 : ((length(qRch) - 1) / 2 + 1)] # Reach outflow (RO)
...
#_______________________________________________________________________________
proc_wq_latQLC <- function(v = NULL, qOut = NULL) {
...
proc_wq_latQLC_by_HRU <- function(strD = NULL, endD = NULL, wqDir = NULL,
  ...
  v <- read_wq_pars('C:/siletz_tmdl/03_models/01_hspf/wq_pars.csv',
                    filN = 'DAVE_UYR', writeCsv = FALSE)
