...
for (i in 1) {
  ...
  source('C:/siletz_tmdl/04_scripts/01_hspf/02_R/fnct_hydro_calib_hspf.R')

  # FILE PATHS ----
  filPath <- 'C:/siletz_tmdl/02_outputs/01_hspf/'
  ...
  qData <- readRDS("C:/siletz_tmdl/02_outputs/01_hspf/base_qOut.RData")

  qData <- qData[['qRch']][, c(1, 12, 4)]

  # Convert m3/s to cfs
  qData[, 2 : 3] <- qData[, 2 : 3] * 35.314666721

  # Read in gage data
  qGage <- read.csv(paste0(datPath, '/gge.csv'), stringsAsFactors = FALSE)
  ...
  # PEAK STORM FLOWS ----
  qStm <- read.csv(paste0(datPath, '/stmObs.csv'), stringsAsFactors = FALSE)
  ...
  write.csv(calSttOut, paste0(pltPath, '/cal_stat_updt.csv'), row.names = FALSE)
